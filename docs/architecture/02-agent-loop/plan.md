...
- [x] 2. **模型表数据与智谱补测**（裁决 A12、M8、A16、A14、A1、E2、H13、M4；owner 的智谱 key 经进程环境，只记变量名）
...
- [x] 4. **测试接缝与官方 key 准备**（裁决 B1、M4、M8、E2、A6）
...
- [ ] 31. **子 agent**（裁决 H5、F7、F2、F3、B3、D11、H11、F5、F11、H13、B1；第一个砍项）
  ...
  - 测试要点：
    ...
    - 旧 196：父会话读过工作区文件、有过 WebSearch 结果，子 agent 去抓一个只出现在 Agent prompt 里、域名已允许的 URL，弹出 flagged 卡；URL 只出现在根会话你亲手写的消息里时同样弹卡（子会话不认根会话的真人消息），出现在父会话 WebSearch 结果的 `searchHitUrls` 里时按域名授权免问；子 agent 读过 .env、抓过网页，交接写回之后，父会话对已允许域名下的非豁免 URL 发起抓取，同样弹出 flagged 卡。
...
- **2026-09-26 · 第 2 步（模型表数据与智谱补测）**，分支 `feat/02-step2-model-table`，完成（format、lint、typecheck、全部单测过；一个提交）。验收 1 全部满足：`TENON_LIVE_ZHIPU_MODEL=glm-5.3-flashx` 下 `pnpm test:live` 的智谱组 2/2 通过（流式回复 3.2 s、停止 6.2 s，2026-09-26）。本步测试要点里只剩 owner 看账单的几项，已在 Open。
  - 改了什么：`ModelInfo.pricing` 只增 `currency?: 'USD' | 'CNY'`（缺省读作 USD）与 `cacheWritePerMTok?`（01 修补 2）。zhipu.ts 四行依次为 glm-5.3、glm-5.3-flash、glm-5.3-flashx、glm-4.6（第一行不动，新用户兜底不变）：都 `supportsToolCalling`、`supportsStreamingToolCalls` 为 true，`thinkingPreservationFormat: 'reasoning-content'`、`reasoningEchoField: 'reasoning_content'`，`requestParams` 为 `{ thinking: { type: 'enabled' }, tool_stream: true }`，不发 `clear_thinking`；flash、flashx 为 1M / 128K、`supportsVision: true`；pricing 见下。文件头注释改为 2026-09-26 读过的页面（flash 两行共用的模型页、定价页与定价落地页、思考页）；「drop」「usageNeedsOptIn」「supportsVision」三条说明按实测改写，新加 `tool_stream` 与 pricing 两条。anthropic.ts 加 `claude-opus-5-5`（1M / 128K，$4 / $20，读缓存 $0.20，写缓存 5 分钟档 $5，不写 `currency`），行序 Sonnet 5、Opus 5.5、Opus 5、Haiku 4.5、Fable 5.1（A16 ownerNote）；改写两处过时注释（行序依据改为 A16；「ModelInfo 没有字段、本步不得发明」改为 02 经 01 修补 2 加 `ThinkingSpec`、第 6 步填，在那之前没有行让 kernel 发思考参数）；Haiku 注释写暂定退役日与 60 天通知；概览页 URL 更新为 `/docs/en/models/overview`，读缓存脚注补上 Opus 5.5 的 5%。ollama.ts 只改注释：`contextLimit` 4096 的依据改为 docs.ollama.com/context-length 的显存分档最低一档（< 24 GiB 4k、24–48 GiB 32k、≥ 48 GiB 256k；FAQ 的 4096 是这一档的确切数），能力位不动。spec 顶部 Revisions 记 flash、flashx 的 `supportsVision` 由 false 定为 true（依据探测 V，旧开放问题 34 关闭），spec 其余不动。思考字段（`thinkingSpec`、档位、`listing`、`display`）不在本步，归第 6 步及以后。
  - 文件：`packages/kernel/src/provider/types.ts`、`packages/kernel/src/provider/definitions/zhipu.ts`、`packages/kernel/src/provider/definitions/anthropic.ts`、`packages/kernel/src/provider/definitions/ollama.ts`、`packages/kernel/test/provider/definitions.test.ts`、`apps/desktop/test/provider-routes.test.ts`、`docs/architecture/02-agent-loop/spec.md`（只有 Revisions 行）、本文件。
  - 定价（2026-09-26 读 docs.bigmodel.cn/cn/guide/start/pricing 与 open.bigmodel.cn/pricing，元 / 百万 token，标准价）：glm-5.3 输入 8、输出 28、缓存命中 2；glm-5.3-flash 0.8、2.8、0.23；glm-5.3-flashx 2、7、0.57。三行都不分档（没有输入长度档，也没有分时段价）。不填 `cacheWritePerMTok`：智谱没有写缓存价，另一项缓存费是按「元 / 百万 Tokens / 小时」计的存储费，目前限时免费、免费期后的价格未公布，不是每百万 token 的写入价。glm-4.6 两个定价页都没有按 token 的价（只有私有实例的算力单元价），不填。落地页 FAQ 写着 flash「限时五折」（输入 0.4、缓存命中 0.115、输出 1.4），没写起止日期、价格卡上没有，表里按标准价，实际单价看账单（Open 里 M8 那条）。
  - Anthropic 各值与 spec 一致：Opus 5.5 的 id、1M / 128K、$4 / $20、读 $0.20、5 分钟写 $5（1 小时档 $8）取自 2026-09-26 的 Opus 5.5 概览页与定价页；其余四行对照各自的页面仍相符；Haiku 4.5 在弃用页上为 Active、退役「不早于 2026-10-15」，当天没有弃用公告，厂商对公开发布的模型「至少提前 60 天通知」。Opus 5 在概览页标为 Legacy（仍可用），弃用页仍为 Active、退役不早于 2027-07-24。
  - 测试要点：旧 87 → definitions.test.ts「the model rows spec 02 changes」：智谱四行的顺序，每行的工具、回传、`requestParams`（恰好两个键，没有 `clear_thinking`）与 `supportsStreamingToolCalls: true`（TS 没有拒收的行），1M / 128K 与 `supportsVision`，pricing（CNY、没有 `cacheWritePerMTok`、glm-4.6 不填）；desktop provider-routes.test.ts 里 `provider.select` 接受 glm-5.3-flash、glm-5.3-flashx，写进 config.json，不返回 `unknown-model`。旧 89 → 同一 describe 里四个用例各经 `zhipuDefinition.create(...)` 取该行、`encode()`：带工具时第二轮 assistant 消息带同一模型的 `reasoning_content`，`thinkingDecisions` 为 echo / same-model；同一段历史不带工具时一条都不回传，为 drop / no-tools；换成另一行产出的思考即使带工具也丢（model-changed）；两种请求体都带 `thinking: {type:'enabled'}` 与 `tool_stream: true`。旧 91 → 原有的三家注册用例加一句 qwen3:8b 的 `supportsToolCalling` 仍为 true。Anthropic 表：同一 describe 断言行序与 Opus 5.5 一行的值（pricing 精确比较，五行都没有 `currency`）。旧 90 → 下面 live 记录的 TS、R46、E2 三行。原有测试只改了 zhipu 用量用例的注释（不再写「待 live 探测」）；acceptance 1 的四家通路用 `models()[0]`，第一行换成 Sonnet 5 后照过。把 glm-4.6 改回 `drop` 并加 `clear_thinking` 试过，新用例变红。
  - live 记录（2026-09-26，owner 的智谱 key）：约定同 spec §实测记录——`ZHIPU_BASE=https://open.bigmodel.cn/api/paas/v4`，每条请求带 `Authorization: Bearer $ZHIPU_API_KEY`（T8 带 `x-api-key: $ZHIPU_API_KEY`）；key 经进程环境交给探针进程，没有打印，没有写进命令字面量或输出，写完后对全部输出文件扫过一遍 key。工具往返用同一个 `get_weather` 函数工具；未注明流式的都是非流式。共 84 次 HTTP 请求（探针 81 次，上限 110；quark 一组 3 次另跑）。探针脚本与原始输出不入库。

    | 编号 | 命令 | 模型 | 状态码 | 关键字段 | 次日账单 |
    |---|---|---|---|---|---|
    | TS | `POST $ZHIPU_BASE/chat/completions`，`stream:true, tool_stream:true, tools:[get_weather], thinking:{type:"enabled"}`；另发同参数不带 `tools` 的「say hi」；glm-5.3、glm-4.6 再各发一次不带 `tool_stream` 的工具请求 | glm-5.3、glm-4.6、glm-5.3-flash、glm-5.3-flashx | 全部 200（10 次） | 带 `tool_stream` 时 `delta.tool_calls[].function.arguments` 分片数：glm-5.3 6、glm-4.6 5、flash 5、flashx 5，拼好都能 `JSON.parse`；不带时 glm-5.3、glm-4.6 都只有 1 片；不带工具、带 `tool_stream:true` 的请求四个模型都 200、`finish_reason=stop`，所以 `requestParams` 里常带 `tool_stream` 安全；四行都判为逐片下发，`supportsStreamingToolCalls` 维持 true；流式不带 `stream_options` 也有 usage | 按 usage |
    | R46 | 同上工具往返，非流式；第二轮 assistant 消息带回首轮 `reasoning_content` 与 `tool_calls`，再发一次去掉 `reasoning_content` 的 | glm-4.6 | 200 ×3 | 首轮 `finish_reason=tool_calls`，`reasoning_tokens` 27；第二轮带回：`finish_reason=stop`，`prompt_tokens` 232；不带：206（差 26，约等于首轮思考）；答案无差别 → glm-4.6 可改 `reasoning-content` | 按 usage |
    | V | `messages[0].content=[{type:"image_url", image_url:{url:"data:image/png;base64,…"}}, {type:"text", text:"What single color fills this image? …"}]`，64×64 纯红、纯蓝 PNG 各一张 | glm-5.3-flash、glm-5.3-flashx；glm-5.3 作对照 | flash、flashx 200 ×4；glm-5.3 400 ×2 | flash、flashx 红、蓝都答对（每张图 `prompt_tokens` 43）；glm-5.3 返回 `1210 messages.content.type 参数非法，取值范围 ['text']`；文档（模型总览、`/cn/guide/models/vlm/glm-5.3-flash`）把 flash、flashx 列为原生多模态（视频、图像、文本、文件），图片可传 URL 或 base64 → 这两行 `supportsVision: true`，glm-5.3、glm-4.6 维持 false（旧开放问题 34 关闭） | 按 usage |
    | E2 | 同一段约 8.2k token 的 system（编号填充行）、同一条 user；依次 A1–A3 `tools:[get_weather,get_time]`，B1–B2 `[get_weather]`，C1 `[get_time,get_weather]`（只换顺序），D1 同 A | glm-5.3-flash | 200 ×7 | `prompt_tokens/cached_tokens`：A1 8252/0，A2 8252/8192，A3 8252/8192，B1 8196/0，B2 8196/8192，C1 8252/0，D1 8252/8192。工具列表增删或只换顺序都让 `cached_tokens` 归零（工具定义在缓存前缀里、排在 system 前），换回原列表仍命中旧缓存；只记录，供阶段 3 在 E2-D 与 E2-F 间选 | 按 usage |
    | EFF | 同类长 system、不带工具，`reasoning_effort` 依次 high、high、low、high | glm-5.3-flash | 200 ×4 | `cached_tokens`：0、8000、0、8000（`prompt_tokens` 8044）。改 `reasoning_effort` 缓存不命中，改回原档又命中旧缓存 → 菜单提示可写「切换会让缓存失效」 | 按 usage |
    | T6+ | 三个工具往返的多回合历史（北京→上海→广州，每回合 user 追问），末条为广州的 `tool` 消息；同一段历史带回全部历史 `reasoning_content` 发两次、全部去掉发两次，另加带回 + `thinking.clear_thinking:false` 一次；flash 再用同一段历史显式带 `clear_thinking:true` 一次 | glm-5.3-flash、glm-5.3 | 200（glm-5.3 途中 4 次 `500 code 1234 网络错误`，每次都挂约 120 s 才返回；北京-0、final-without-1 重发即过，上海-0 重发仍 500，整组以 T6+b 重跑后过；带不带回传都遇到过） | flash：带回 485/485，不带 422/422（差 63 = 三段思考 21+20+22），`clear_thinking:false` 485，显式 true 也是 485——flash 计入全部回传思考、`clear_thinking` 不起作用；`cached_tokens` 全 0；耗时 7.3/7.1 s 对 7.2/6.4 s。glm-5.3（默认 `clear_thinking` 为 true）：带回 448/448，不带 424/424（差 24 = 只有本回合广州那段），`clear_thinking:false` 495（差 71 = 26+21+24）；`cached_tokens` 带回 320/384，不带 320/384，false 只有 192；耗时 4.7/4.2 s 对 5.7/2.5 s，看不出规律；答案无差别 | 按 usage（owner 2026-09-26 定 A，不另核） |
    | INT | 首轮问北京天气 → `tool_calls`；再发 [user, assistant(`reasoning_content`+`tool_calls`), tool(北京结果), user「顺便也查一下上海的天气」]，`thinking:{type:"enabled", clear_thinking:true}` 与 `false` 各两次；每次再答完上海的调用、发一轮追答 | glm-5.3 | 200 ×9 | 插话轮 `prompt_tokens`：true 215/215，false 241/241（差 26 = 首轮思考）；`cached_tokens`：true 128/192，false 192/192；插话轮思考长度：true 48/75，false 20/31；四次都 `finish_reason=tool_calls`、调 `get_weather{"city":"上海"}`，推理有三次写到「已拿到北京天气」（false-1 全文只有 "Shanghai's weather too"，没提北京结果）；追答四次都给出两城天气（false-1 主列上海、对比里带北京）；追答 `prompt_tokens` 符合 254 +（false 时 26）+ 插话轮思考。推理没有明显断掉，本次样本不支持「推迟插话」或「插话时强制 false」 | 按 usage |
    | MCP | 未跑 | — | — | 对话补全文档写了 `tools:[{type:"mcp", mcp:{server_label, server_url?, transport_type, allowed_tools, headers}}]`（`mcp` 里只有 `server_label` 必填），接智谱托管的 server 时 `server_label` 填「mcp code」、可不填 `server_url`，但没列出普通 key 可用的 code；文档列出的四个智谱托管 MCP（联网搜索、网页读取、开源仓库、视觉理解）都是 GLM Coding Plan 专属、要套餐 key，Tenon 不得用 | — |
    | T8 | `POST https://open.bigmodel.cn/api/anthropic/v1/messages`，`x-api-key: $ZHIPU_API_KEY`、`anthropic-version: 2023-06-01`，非流式：(a) 纯文本；(b) 顶层 `cache_control` + 约 8k system ×2；(c) system 块级 `cache_control` ×2；(d) `get_weather` 的 tool_use 往返；(e) `web_search_20250305` 服务端工具；(f) `thinking:{type:"adaptive"}`；(g) `output_config:{effort:"high"}` 单独发、加 adaptive 各一次；(h) adaptive + `display:"summarized"`；(i) `thinking:{type:"enabled", budget_tokens:1024}`；另发不带 `cache_control` 的对照 ×2 | glm-5.3 | 全部 200（15 次） | 不传 `thinking` 也回 `thinking` 块（c 的第二次例外），每个 thinking 块都带 24 字符的 `signature`；usage 只有 `input_tokens`、`output_tokens`、`cache_read_input_tokens`、`server_tool_use.web_search_requests`、`service_tier`，没有 `cache_creation_input_tokens`；(b)(c) 第二次 `cache_read_input_tokens=8000`、`input_tokens=47`，不带 `cache_control` 的对照同样是 8000/46——`cache_control` 收下但不起作用，缓存是隐式的；(d) `stop_reason=tool_use`，原样带回 thinking+tool_use 并给 `tool_result` 后 `end_turn`；(e) 200 但没有 `server_tool_use`、`web_search_tool_result` 块，`web_search_requests=0`，模型答「没有联网搜索功能」，属静默忽略；(f)–(i) 全部收下、都回 thinking+text，`display`、`effort` 看不出效果 | 按 usage |
    | Q | `POST $ZHIPU_BASE/web_search`，请求体照 spec §智谱后端：`search_query`、`search_engine:"search_pro_quark"`、`search_intent:false`，不传 `count`；再把命中映射成 `{title, link, content, publish_date}` 的 JSON 作 `tool` 消息，给带工具的 glm-5.3-flashx 发一次，另发一次 `tool` 内容为 `[]` 的对照 | —；glm-5.3-flashx | 200 ×3 | 默认返回 10 条、10 条都带 `link`；结果 JSON 5991 个码点；`prompt_tokens` 带结果 2660、空结果 194，差 2466（约 2.4 码点 / token），`cached_tokens` 都为 0。一次搜索结果进上下文约 2.5K token | 搜索 ¥0.05 / 次待 owner 核（S1 那条） |

  - 结论：
    - TS：四行都逐片下发工具参数，没有拒收 `tool_stream` 的行，所以四行都带 `tool_stream: true`、`supportsStreamingToolCalls` 都为 true。API 参考的视觉模型 schema（flash 两行在里面）没列 `tool_stream`，flash 模型页却建议开，以实测为准。
    - R46：glm-4.6 收回传的 `reasoning_content`，与其余三行一样改 `reasoning-content`。
    - V：flash、flashx 看得见图，`supportsVision: true`；glm-5.3 拒收图片，glm-4.6 文档只写文本，两行维持 false。已记 spec Revisions（2026-09-26）。
    - E2：只改工具列表（连只换顺序）就让 `cached_tokens` 归零，换回原列表又命中旧缓存，几份缓存可以并存；缓存以 64 token 为单位计。只记录，阶段 3 在 E2-D 与 E2-F 之间选时用。
    - EFF：改 `reasoning_effort` 会让隐式缓存不命中，改回原档又命中；菜单写「切换会让缓存失效」属实（第 19 步的措辞）。
    - T6+：glm-5.3 默认 `clear_thinking` 为 true，服务端丢掉历史回合的思考，回传只多算本回合那段，前缀缓存照样命中；flash 计入每一段回传的历史思考，`clear_thinking` 取 true、false 都一样。账单若按 `prompt_tokens` 计，glm-5.3-flash 每次调用都要为全部历史思考再付一次输入费（flashx 未测，2026-09-25 的 T6 只有单回合，分不出来），只有 D（客户端丢历史思考的守卫）能免；C（`clear_thinking:false`）在 glm-5.3 上加钱、在 glm-5.3-flash 上不起作用。**owner 2026-09-26 定 A**（不改）：智谱按接口返回的 usage 计费，usage 已经说明 flash 计入回传的历史思考，不必再等账单（差额每次几十 token，账单精度看不出）；钱很少（估算：前面 20 个回合、每回合约 500 token 思考，每次请求多算约 1 万 token，flash 上不到 1 分钱），且 flash 模型页本身建议 `clear_thinking: false` 保留旧思考，flash 现在的行为正合厂商建议；D 要改 01 的守卫，C 只让 glm-5.3 多花钱。spec 取厂商默认值，不改。
    - INT：`clear_thinking` 取 true、false，插话后推理都没有明显断掉：true 时从工具结果里重建状态（少 26 个输入 token，思考更长），false 时接着想、更短。本次样本（每组 2 次、简单任务）不支持「推迟插话」或「插话时强制 false」。推理没有明显断掉，spec §验收标准 开头的条件不成立，H13 的智谱插话规则照现写法，不走 Revisions；插话请求的 `clear_thinking` 随 T6 的选择走（选 C 时各行 `requestParams` 带 false，插话同样带）。
    - MCP：没跑。普通 key 能用的 mcp code 文档里没有，列出的四个智谱托管 MCP 都要 GLM Coding Plan 的 key，Tenon 不得用。
    - T8：`/api/anthropic` 收下 `cache_control`（顶层、块级）但不起作用，缓存是隐式的，也不报 `cache_creation_input_tokens`；服务端 `web_search_20250305` 被静默忽略；`thinking` 的 adaptive / enabled、`display`、`output_config.effort` 都收下，看不出效果；不传 `thinking` 也回带 24 字符 `signature` 的 thinking 块，原样带回 thinking+tool_use 的往返能过。评测专用 glm-5.3 行怎么填，留给用到它的那一步。
    - 另记：glm-5.3 在 09:40–09:49（北京时间）遇到 4 次 `500 code 1234 网络错误，请稍后重试`，每次都挂约 120 秒才返回 500，其中一次重发仍 500，重发或整组重跑都过了，带不带回传都遇到过——供第 7 步的错误映射（1234 应当可重试）。没有遇到额度或计费错误（1113、1308、1310、1316），Open 里抓 1308、1310 报文那条不变。
...
- **2026-09-26 · 第 4 步（测试接缝与官方 key 准备）**，分支 `wt/02-step4`，不要官方 key 的部分完成（format、lint、typecheck、全部单测过；一个提交）。本步没有验收；要官方 key 的实测见下面「没做的」，第 33 步补。
  - 改了什么：`fakeNetwork(script, options?)` 只增可选的 `FakeNetworkOptions { checkRequest?, untrusted? }`，形状照 spec 01 修补 4 的代码块；`FakeNetwork` 只增 `checkFailures`、`untrustedRequests` 与 `fetchUntrusted`。`checkRequest` 在每次 `fetch` 记下请求之后、回放与任何拒绝（读不出的 body、已中止的 signal、脚本用完）之前运行，抛出的值按调用顺序进 `checkFailures`，这次 fetch 照常回放；`fetchUntrusted` 用自己的脚本、游标和请求记录，不跑 `checkRequest`，没传 `untrusted` 时先记下请求再抛错。`HostNetwork`（host/adapter.ts）与内存 host 不动，`HostNetwork.fetchUntrusted` 归第 27 步。新文件 `packages/kernel/src/testing/request-assertions.ts`，经 `@tenon-app/kernel/testing` 导出四个请求体断言，各自对应一条不变量：`assertToolPairing`（不变量 23）、`assertLastTurnIsUser`（2；openai-chat 线以 `role: 'tool'` 结尾也算 user 轮，那是工具结果的 user 轮）、`assertHeaderNamesAllowed(request, allowed)`（3；白名单由调用方传入，集合、列表或谓词，第 7 步的白名单函数可直接当谓词传）、`assertImagesInline`（10；Anthropic 的 `image` 块只许 `base64` 来源，`image_url` 只许 `data:`，搜遍整个 body，含工具结果里的图）；另导出 `requestWire`（按 URL 路径分线，`…/messages` 与 `…/chat/completions`，其余为 null）与 `RequestAssertionError`（`problems` 逐条列出，只写下标、id 与头名，不写头的值）。`assertToolPairing` 按线分：Anthropic 查每个 direct 的 `tool_use` 在紧随的 user 消息（连续的 user 消息算同一轮）里恰有一个 `tool_result`，且排在该轮第一个其他块之前；openai-chat 查每个 `tool_calls[]` 在紧随的一串 `role: 'tool'` 消息里恰有一条；两个方向都查（答不上紧前一轮调用的结果也报），同一轮重复的调用 id 也报；`server_tool_use`、`mcp_tool_use`、`caller` 不是 direct 的 `tool_use`、`type: 'mcp'` 的调用不要求结果。两个按线的断言遇到别的路径直接抛错，不放过——`checkRequest` 同时会看到搜索请求时，先按 `requestWire(request) !== null` 筛（写在模块头注释里）。
  - 不变量测试命名：测试名里写「02 不变量 N」，每个编号写全（`02 不变量 3, 02 不变量 10`，不写 `02 不变量 3、10`），第 35 步用 `grep -rnE '02 不变量 N([^0-9]|$)'` 找；约定写在 `packages/kernel/src/testing/index.ts` 文件头。上面几个断言自己的单测不带这个标记（它们证明的是断言，不是 kernel）。
  - `liveEnv()` 拆开：live 套件里 Anthropic 线分两组，互不共用 key。仿真组（「live provider · anthropic emulation」，glm-4.7-flash 走智谱 `/api/anthropic`）只转交 `ANTHROPIC_BASE_URL` 与 `ANTHROPIC_AUTH_TOKEN`（及 `TENON_MODEL`、`TENON_MAX_TOKENS`、`TENON_PROVIDER=anthropic`），不读 `ANTHROPIC_API_KEY`；token 形如官方 key（`sk-ant-` 开头）、没设 base URL、或 base URL 在 `api.anthropic.com` 时整组报错不跑，没配 token 时跳过（原来是报错）。官方组（「live provider · anthropic official」）的 key 只从进程环境的 **`TENON_LIVE_ANTHROPIC_OFFICIAL_KEY`** 读，模型用 `TENON_LIVE_ANTHROPIC_OFFICIAL_MODEL`（不设就是 anthropic 定义的第一行：`TENON_MODEL` 总是由本组设，不设时为空串，desktop 把空当未配置；运行者 shell 里导出的 `TENON_MODEL` 因此进不来）；以 `ANTHROPIC_API_KEY` 启动，不带 `ANTHROPIC_BASE_URL` 与 `ANTHROPIC_AUTH_TOKEN`，只能落到定义的默认端点 `api.anthropic.com`；没设就跳过，`.env.local` 里出现这个变量或任何 `sk-ant-` 开头的值时整组报错。两组可在同一次 `pnpm test:live` 里并存。靠构造保证：e2e 启动环境的拼装从 `launchTenon` 抽成纯函数 `appEnvironment`（`apps/desktop/e2e/helpers/app-env.ts`），对**所有** e2e 启动都不继承运行者环境里的 `ANTHROPIC_API_KEY`、`ANTHROPIC_AUTH_TOKEN`、`ANTHROPIC_BASE_URL`、`ZHIPU_API_KEY`、`TENON_LIVE_ANTHROPIC_OFFICIAL_KEY`（与原来的 `ELECTRON_RUN_AS_NODE`），测试的凭据只经 `options.env`；拼好后若 `ANTHROPIC_API_KEY` 或 `ANTHROPIC_AUTH_TOKEN` 形如官方 key 而 `ANTHROPIC_BASE_URL` 指向别的主机，拒绝启动（报错只写变量名）。两个 Anthropic 组都改用内存机密接缝：真钥匙串里的机密先于环境变量读（apps/desktop/src/main/provider.ts 的 `readProviderInputs`），日常经设置卡存进钥匙串的官方 key 会跟着仿真组的 base URL 发出去；真钥匙串路径仍由智谱组覆盖（它选的是 zhipu，发出去的只有 zhipu 的机密）。文件头注释改写：运行方法按三组写，「WHAT THE FIRST RUNNER OWES」删去已由 01 live 记录（2026-09-22）了结的 `usageNeedsOptIn` 与智谱夹具两项，只留「官方组第一次跑时拿一条真实 Anthropic 流与手写夹具逐帧对照」。
  - 文件：`packages/kernel/src/testing/fake-network.ts`、`packages/kernel/src/testing/request-assertions.ts`（新）、`packages/kernel/src/testing/index.ts`、`packages/kernel/test/testing/fake-network.test.ts`、`packages/kernel/test/testing/request-assertions.test.ts`（新）、`apps/desktop/e2e/helpers/app-env.ts`（新）、`apps/desktop/e2e/helpers/live-env.ts`（新）、`apps/desktop/e2e/helpers/launch.ts`、`apps/desktop/e2e/live-provider.spec.ts`、`apps/desktop/test/live-env.test.ts`（新）、本文件。
  - 测试要点：「原有的 `fakeNetwork(script)` 与 `requests`、`callCount` 照旧」→ fake-network.test.ts 原有 20 个用例一字未改全过，另加「keeps the one-argument form exactly as it was」（不传选项时两个新列表为空、耗尽报错仍是 `call N`）；「`checkRequest` 抛错记进 `checkFailures`、回放照常」→ 同文件「runs checkRequest on every fetch before playback…」（529 后的重发照常回放、只记一条）与「checks calls that never reach playback too」（已中止与脚本用完的调用也检查），request-assertions.test.ts「as a checkRequest behind a real provider」用真的 `AnthropicMessagesProvider` 发一段缺结果的历史，流照常走到 `stop{end-turn}`，`checkFailures` 恰一条；`untrusted` 分开计数、没脚本时抛错 → 「replays the untrusted script with its own cursor and request log」「makes fetchUntrusted reject every call…」。`assertToolPairing` 两条线都先对编码器的真实输出（两轮、含并行两个调用与尾随文字）通过，再逐个坏形状失败：缺结果、以调用结尾、答两次、结果排在 user 文字后（同一条与下一条消息）、答不上紧前一轮、晚一轮才答、重复 id，openai-chat 另有结果排在下一条 user 之后；服务端执行的四种块不要求结果；非模型路径与没有 messages 的 body 抛错。其余三个断言各有通过与失败用例（末轮是 assistant、没有消息、Anthropic 线的 `tool` 角色；白名单外的头名逐个列出且报错里没有头的值；URL 与 file 来源的图、嵌在工具结果里的图、非 `data:` 的 `image_url`）。live-env.test.ts 11 个用例：所有启动都不继承上述变量（并核对它覆盖 `DEV_ENV_FALLBACK` 的每个变量名）、守卫拒绝与放行（官方形 key 取 `sk-ant-api03-`、`sk-ant-oat01-`、`sk-ant-admin01-` 三种，外部 base URL 含仿官方主机 `api.anthropic.com.evil.test`、`evil.test/api.anthropic.com`、`proxy.anthropic.com` 与不带 scheme 解析不了的一条）、仿真组不带官方 key（只有 `ANTHROPIC_API_KEY` 或官方 key 变量时跳过，不拿它们补 token；仿官方主机算外部端点照常跑）、官方组只带 key 不带 base URL、运行者 shell 的 `TENON_MODEL` 带不进官方组、`.env.local` 里有 key 时拒绝（按变量名，值不像官方 key 也拒；或任何变量名下的三种官方形值）、两组并存时互不见对方的 key。变异自测：kernel 两个文件 25 处、e2e 两个 helper 15 处逐一改坏，40 处里 39 处让对应用例变红；剩下一处（官方组改为也从 `.env.local` 读 key）是等价变异——文件里有 key 时先被拒绝，读不到那一步。评审另找到 11 处漏网的变异（`isOfficialBaseURL` 的四种改法、官方形前缀收窄到 `sk-ant-api` 的两处、仿真组 token 查找末尾追加 `ANTHROPIC_API_KEY` 或官方 key 变量、去掉 `.env.local` 的按变量名拒绝、openai-chat 成对一轮里夹一条孤立 tool 消息的两种改法）和一处缺陷（官方组的 `TENON_MODEL` 从运行者 shell 继承）；补上用例、修了缺陷后，这 12 处都让用例变红。另跑了 `pnpm build` 后的 smoke、chat、provider-settings 三个 e2e（10/10 过，`launchTenon` 改了环境拼装），以及 `TENON_LIVE=1 playwright test --config=playwright.live.config.ts --list`（8 个用例照常收集，没有运行）；没跑 `pnpm test:live`。
  - 没做的（都要官方 key，按 spec §模型与密钥 不试）：判账号类型（方法见第 33 步）；Anthropic 线同一 provider 换到不发工具的模型、历史有 tool_use / tool_result 时会不会 400（最好在第 10 步开工前出结论；第 10 步之后才出、又要补的按 (b)，开放问题 20）；搜索子请求的三个参数（第 28 步）；老账号的补头接缝只在判定为老账号时才建，现在不建，它的专用变量名到时在这里定；官方组第一次跑时对照 Anthropic 线的手写 SSE 夹具（01 plan.md 的 live 记录）。这些都等 Open 里「Anthropic 官方 key」那条。
